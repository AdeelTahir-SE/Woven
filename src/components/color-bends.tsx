...
void main() {
  ...
  if (uColorCount > 0) {
    ...
    for (int k = 0; k < 3; ++k) {
      ...
      float m = mix(m0, m1, kMix);
      float channel = 1.0 - exp(-uBandWidth / exp(uBandWidth * m));
      if (k == 0) col.r = channel;
      else if (k == 1) col.g = channel;
      else col.b = channel;
    }
...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < MAX_COLORS; i += 1) {
      if (i < arr.length) uniforms[i].copy(arr[i]);
      else uniforms[i].set(0, 0, 0);
    }
